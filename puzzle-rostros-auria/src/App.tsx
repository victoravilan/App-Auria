...
type Tab = "home" | "game" | "cam" | "tts" | "tp" | "abc" | "dict";
      ...
      {!isInstalled && (
        ...
        >
          <div className="grid grid-cols-5">
            <NavBtn icon="PZ" label="Puzzle" active={tab === "game"} onClick={() => setTab("game")} />
            <NavBtn icon="ðŸ " label={t(lang.code, "home")}        active={tab === "home"} onClick={() => setTab("home")} />
            <NavBtn icon="ðŸ“·" label={t(lang.code, "camToText")}   active={tab === "cam"}  onClick={() => setTab("cam")} />
            <NavBtn icon="ðŸ“–" label={t(lang.code, "dictionary")}  active={tab === "dict"} onClick={() => setTab("dict")} />
            <NavBtn icon="ðŸ”¤" label={t(lang.code, "alphabet")}    active={tab === "abc"}  onClick={() => setTab("abc")} />
          </div>
        </nav>
      )}
              ...
              {isIOS ? (
                ...
                <>
                  <li className="flex items-center gap-3 rounded-xl bg-white/5 p-3">
                    ...
                    <span>Abre el menÃº del navegador (â‹®)</span>
                  </li>
                  <li className="flex items-center gap-3 rounded-xl bg-white/5 p-3">
                    <span className="text-2xl">â¬‡ï¸</span>
                    <span>Toca â€œInstalar aplicaciÃ³nâ€ o â€œAÃ±adir a pantalla de inicioâ€</span>
                  </li>
                  <li className="flex items-center gap-3 rounded-xl bg-white/5 p-3">
                    <span className="text-2xl">âœ…</span>
    ...
    <div className="flex flex-col gap-5">
      ...
      <section className="glass relative overflow-hidden rounded-3xl p-6 text-center">
        ...
        <div className="relative">
          ...
          >â–¶ {t(lang.code, "cta")}</button>
        </div>
      </section>

      {/* Install banner â€” only if installable and NOT installed */}
      {canInstall && (
        <button
          onClick={onInstall}
        ...
        >
          <div className="absolute -right-6 -top-6 text-8xl opacity-20 transition group-hover:scale-110">ðŸ“²</div>
          <div className="relative flex items-center gap-3">
            <div className="grid h-12 w-12 place-items-center rounded-2xl bg-white/20 text-2xl backdrop-blur">â¬‡</div>
            <div className="flex-1">
      ...
      <section className="grid grid-cols-2 gap-3">
        <Tile color="from-sky-500 to-cyan-600" icon="PZ" label="Puzzle de emociones" onClick={() => go("game")} />
        <Tile color="from-sky-500 to-indigo-600" icon="ðŸ“·" label={t(lang.code, "camToText")} onClick={() => go("cam")} />
        <Tile color="from-fuchsia-500 to-pink-600" icon="âœ‹" label={t(lang.code, "textToSign")} onClick={() => go("tts")} />
        <Tile color="from-violet-500 to-purple-600" icon="ðŸ“–" label={t(lang.code, "dictionary")} onClick={() => go("dict")} />
        <Tile color="from-emerald-500 to-teal-600" icon="ðŸ”¤" label={t(lang.code, "alphabet")} onClick={() => go("abc")} />
        <Tile color="from-amber-500 to-orange-600" icon="ðŸ“º" label={t(lang.code, "teleprompter")} onClick={() => go("tp")} />
        <Tile color="from-rose-500 to-red-600" icon="ðŸ’¡" label={t(lang.code, "suggest")} onClick={() => go("dict")} />
      </section>

      {/* About */}
      <section className="glass rounded-3xl p-5">
        <h2 className="mb-2 text-lg font-bold">ðŸ“š {t(lang.code, "aboutTitle")}</h2>
